...
def train_crm(df, tau_selection = 'per-pair', constraints = 'up-to one'):
    ...
    data1 = dict(x=df['Time'].values, y=df[production_wells].values)
    data2 = dict(x=df_train["Time"].values, y=q_train)
    data3 = dict(x=df_validate["Time"].values, y=q_test)
    data4 = dict(x=df['Time'].values, y=df[injection_wells].values)
