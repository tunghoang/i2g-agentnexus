...
class CRMIP: 
    def __init__(self, inputs_list, include_press=False):
        # tau is now N_inj x N_prd (pairwise)
        self.tau = inputs_list[0]             # shape (N_inj, N_prd)
        self.gain_mat = inputs_list[1]        # shape (N_inj, N_prd)
        self.N_inj = self.gain_mat.shape[0]
        self.N_prd = self.gain_mat.shape[1]
        self.qp0 = inputs_list[2]             # shape (N_prd,)
    ...
    def prim_prod(self):
        ...
        # For CRMIP: primary production uses injector-specific tau averaged by gains
        denom = np.sum(self.gain_mat / (tau + 1e-8), axis=0)
        effective_tau = 1.0 / np.maximum(denom, 1e-8)

        q_prime = q_prev * np.exp(-del_t / effective_tau)

        self.q = q_prime
        return q_prime

    def inject_term(self):
        tau = self.tau
        del_t = self.del_t
        ...
        # Pairwise tau decay
        b = (1 - np.exp(-del_t / tau))  # shape (N_inj, N_prd)
        q_inj = np.sum(qi_t.reshape(-1,1) * gain_mat * b, axis=0)
    ...
    def bhp_term(self):
        ...
        effective_tau = np.sum(self.gain_mat / tau, axis=0)**(-1)
        q_bhp = -(J * effective_tau * del_bhp_t / del_t) * (1 - np.exp(-del_t / effective_tau))
    ...
    def compute_grad_tau(self):
        ...
        grad_tau = (q_prev.reshape(1,-1) * a) - (qi_t.reshape(-1,1) * gain_mat * a)
    ...
    def compute_grad_lambda(self):
        ...
        b = (1.0 - np.exp(-del_t/tau))  # shape (N_inj, N_prd)
        grad_lambda = qi_t.reshape(-1,1) * b
    ...
    def compute_grad_q0(self):
        ...
        self.grad_q0 = np.exp(-(t - t0) / effective_tau)
    ...
    def compute_grad_J(self):
        ...
        self.grad_J = effective_tau * del_bhp_t / del_t
    ...
    def prod_pred(self, input_series, train=False):
        ...
        if train:
            self.Grad_Tau = np.array(grad_taus)
    ...
    def compute_grads(self, q_obs):
      ...
      residuals = (q_obs[1:,:] - q_pred[1:,:])  # (T, N_prd)

      # --- Fix for tau gradient ---
      dmse_dtau = 2 * np.sum(residuals[:, None, :] * -Grad_Tau, axis=0) / np.max(q_obs, axis=0)**2

      dmse_dlambda = []
      for i in range(self.N_inj):
          dmse_dlambda.append(
              2*np.sum(residuals * -Grad_Lambda[:,i,:], axis=0) / np.max(q_obs,axis=0)**2
          )
    ...
    def obj_func_fit(self,x,input_series,q_obs):
        ...
        self.tau = x[:(N_prd*N_inj)].reshape(N_inj, N_prd)
        self.gain_mat = x[(N_prd*N_inj):(2*N_prd*N_inj)].reshape(N_inj, N_prd)
        self.q0 = x[(2*N_prd*N_inj):(2*N_prd*N_inj+N_prd)]
        if self.include_press:
            self.J0 = x[(2*N_prd*N_inj+N_prd):]
    ...
    def fit_model(self,input_series, q_obs, init_guess):
        ...
        tau_0 = init_guess[0]      # shape (N_inj, N_prd)
        ...
        bnds = []
        # tau bounds
        for i in range(N_inj*N_prd):
            bnds.append((0.00000001, 100000))
        # gain_mat bounds
        for i in range(N_inj*N_prd):
            bnds.append((0.0, 1.0))
        # q0 bounds
        ...
        res = minimize(self.obj_func_fit, x0,args=(input_series,q_obs),
                    ...
                    'ftol': 1e-6,'maxiter':100})
    
        fit_param = res.x
        tau_fit = fit_param[:(N_prd*N_inj)].reshape(N_inj, N_prd)
        gain_mat_fit = fit_param[(N_prd*N_inj):(2*N_prd*N_inj)].reshape(N_inj, N_prd)
        q0_fit = fit_param[(2*N_prd*N_inj):(2*N_prd*N_inj+N_prd)]
        param_fits = [tau_fit, gain_mat_fit, q0_fit]
        if self.include_press:
            J_fit = fit_param[(2*N_prd*N_inj+N_prd):]
    ...
    def apply_gain_mat_constraint(self,x):
        N_prd  = self.N_prd
        N_inj =  self.N_inj
        gain_mat = x[(N_prd*N_inj):(2*N_prd*N_inj)].reshape(N_inj,N_prd)
